#!/usr/bin/env python
"""
Verifica a aba ANUAL da planilha de Disponibilidade
"""
...
def verificar_aba_anual_disponibilidade():
    """Verifica a aba ANUAL da planilha de Disponibilidade"""
    
    print("🔍 Verificando aba ANUAL da planilha de Disponibilidade...")
    ...
    try:
        ...
        # Acessar aba ANUAL
        try:
            anual_ws = sheet.worksheet('ANUAL')
            print(f"📊 Aba ANUAL: {anual_ws.row_count} linhas x {anual_ws.col_count} colunas")
        except:
            print("❌ Aba ANUAL não encontrada")
            return False
        
        # Obter cabeçalhos
        headers = anual_ws.row_values(1)
        ...
        if not colunas_aprovacao:
            ...
            for i, header in enumerate(headers, 1):
                if header:
                    ...
                    try:
                        valores = anual_ws.col_values(i)[1:]  # Pular cabeçalho
        ...
        if colunas_aprovacao:
            ...
            for col in colunas_aprovacao:
                ...
                try:
                    valores = anual_ws.col_values(col)[1:]  # Pular cabeçalho
                    contador = Counter(valores)
        ...
        print(f"\n📋 AMOSTRA DE DADOS COMPLETOS (primeiras 10 linhas):")
        try:
            dados_amostra = anual_ws.get_values('A1:AC11')  # Primeiras 11 linhas, colunas A-AC
...
if __name__ == "__main__":
    verificar_aba_anual_disponibilidade()
